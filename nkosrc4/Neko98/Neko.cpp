...
CNeko::CNeko( char* lpszName )
{
	...
	if( NekoSettings.IsOpen() )
	{
		...
		NekoSettings.GetString( szNekoLibraryKey,	m_lpszLibname, MAX_PATH-1 );
	...
	if( m_lpszLibname == NULL || *m_lpszLibname == '\0' || ((int)ExtractIcon( g_hInstance, m_lpszLibname, -1 ) < 32 ))
    {
        //use default images if there is no file or not enough icons
		GetModuleFileName( NULL, m_lpszLibname, MAX_PATH );
...
void CNeko::RunTowards(int nX, int nY)
{
    ...
    switch( m_State ) 
    {
        ...
        case DR_MOVE:
		{
			...
			int nNewX = m_pPet->GetPosition().x + m_nDX, nNewY = m_pPet->GetPosition().y + m_nDY;
            ...
            BOOL fNotMoved = ( nNewX == m_pPet->GetPosition().x ) && ( nNewY == m_pPet->GetPosition().y );
...
BOOL CNeko::LoadImages()
{
	...
	*/
	...
	HICON hIcons[32];
	for( n = 0; n < 32; n++ )
		hIcons[n] =  ExtractIcon( g_hInstance, m_lpszLibname, n );
